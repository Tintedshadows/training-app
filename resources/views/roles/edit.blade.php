  ...
  <div class="row">
    <div class="col-md-12">
      ...
      <form method="POST" action="http://localhost:8080/training/public/roles/edit/{{$data->id}}" accept-charset="UTF-8"><input name="_token" type="hidden" value="M2r2P3RpYQvXqSeK8iNmOBToWUX7IEDSIkOzFcmm">
        ...
        <input name="createRole" type="checkbox" @if($data->createRole === 1)checked value="1" @endif @if($data->createRole === 0)value="0" @endif><br/>

        <label for="deleteEm">Delete Employees</label>
        <input name="deleteEm" type="checkbox" @if($data->deleteEm === 1)checked value="1" @endif @if($data->deleteEm === 0)value="0" @endif>

        <label for="deleteRole">Delete Roles</label>
        <input name="deleteRole" type="checkbox" @if($data->deleteRole === 1)checked value="1" @endif @if($data->deleteRole === 0)value="0" @endif><br/>

        <label for="deleteUsers">Delete Users</label>
        <input name="deleteUsers" type="checkbox" @if($data->deleteUsers === 1)checked value="1" @endif @if($data->deleteUsers === 0)value="0" @endif>
